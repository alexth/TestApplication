...
import static com.example.widget.SeekBar.INDICATOR_ALWAYS_HIDE;
import static com.example.widget.SeekBar.INDICATOR_ALWAYS_SHOW;
...
public class RangeSeekBar extends View {
    ...
    private void initProgressBitmap() {
        if (progressBitmap == null) {
            progressBitmap = new SeekBarUtils().drawableToBitmap(getContext(), progressWidth, progressHeight, progressDrawableId);
        }
        if (progressDefaultBitmap == null) {
            progressDefaultBitmap = new SeekBarUtils().drawableToBitmap(getContext(), progressWidth, progressHeight, progressDefaultDrawableId);
        }
    }
    ...
    private void initStepsBitmap() {
        ...
        if (stepsBitmaps.isEmpty()) {
            Bitmap bitmap = new SeekBarUtils().drawableToBitmap(getContext(), (int) stepsWidth, (int) stepsHeight, stepsDrawableId);
    ...
    private void initAttrs(AttributeSet attrs) {
        try {
            ...
            progressHeight = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_progress_height, new SeekBarUtils().dp2px(getContext(), 2));
            ...
            tickMarkTextMargin = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_margin, new SeekBarUtils().dp2px(getContext(), 7));
            tickMarkTextSize = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_size, new SeekBarUtils().dp2px(getContext(), 12));
    ...
    protected int getTickMarkRawHeight() {
        if (tickMarkTextArray != null && tickMarkTextArray.length > 0) {
            return tickMarkTextMargin + new SeekBarUtils().measureText(String.valueOf(tickMarkTextArray[0]), tickMarkTextSize).height() + 3;
        }
        return 0;
    }
        ...
        if (tickMarkTextArray != null) {
            ...
            for (int i = 0; i < tickMarkTextArray.length; i++) {
                ...
                if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
                    ...
                    float num = new SeekBarUtils().parseFloat(text2Draw);
                    SeekBarState[] states = getRangeSeekBarState();
                    if (new SeekBarUtils().compareFloat(num, states[0].value) != -1 && new SeekBarUtils().compareFloat(num, states[1].value) != 1 && (seekBarMode == SEEKBAR_MODE_RANGE)) {
    ...
    protected void onDrawProgressBar(Canvas canvas, Paint paint) {
        ...
        if (new SeekBarUtils().verifyBitmap(progressDefaultBitmap)) {
        ...
        if (new SeekBarUtils().verifyBitmap(progressBitmap)) {
    ...
    /**
     * @return the two seekBar state , see {@link com.example.widget.SeekBarState}
     */
    public SeekBarState[] getRangeSeekBarState() {
        ...
        if (new SeekBarUtils().compareFloat(leftSeekBarState.value, minProgress) == 0) {
            leftSeekBarState.isMin = true;
        } else if (new SeekBarUtils().compareFloat(leftSeekBarState.value, maxProgress) == 0) {
        ...
        if (seekBarMode == SEEKBAR_MODE_RANGE) {
            ...
            if (new SeekBarUtils().compareFloat(rightSB.currPercent, minProgress) == 0) {
                rightSeekBarState.isMin = true;
            } else if (new SeekBarUtils().compareFloat(rightSB.currPercent, maxProgress) == 0) {
    ...
    public void setStepsDrawable(List<Integer> stepsDrawableIds) {
        ...
        for (int i = 0; i < stepsDrawableIds.size(); i++) {
            Bitmap bitmap = new SeekBarUtils().drawableToBitmap(getContext(), (int) stepsWidth, (int) stepsHeight, stepsDrawableIds.get(i));
            stepsBitmaps.add(bitmap);
        }
